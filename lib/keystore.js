...
function encryptString (string, pwDerivedKey) {
  ...
  var encObj = nacl.secretbox(util.decodeUTF8(string), nonce, pwDerivedKey);
  var encString = { 'encStr': util.encodeBase64(encObj),
...
KeyStore._decryptString = function (encryptedStr, pwDerivedKey) {
  ...
  var secretbox = util.decodeBase64(encryptedStr.encStr);
  var nonce = util.decodeBase64(encryptedStr.nonce);
  ...
  return util.encodeUTF8(decryptedStr);
};
...
KeyStore._encryptKey = function (privKey, pwDerivedKey) {
  ...
  encKey = { 'key': util.encodeBase64(encKey), 'nonce': util.encodeBase64(nonce)};
...
KeyStore._decryptKey = function (encryptedKey, pwDerivedKey) {
  ...
  var secretbox = util.decodeBase64(encryptedKey.key);
  var nonce = util.decodeBase64(encryptedKey.nonce);
...
KeyStore._computePubkeyFromPrivKey = function (privKey, curve) {
  ...
  var privKeyUInt8Array = util.decodeBase64(privKeyBase64);
  var pubKey = nacl.box.keyPair.fromSecretKey(privKeyUInt8Array).publicKey;
  var pubKeyBase64 = util.encodeBase64(pubKey);
